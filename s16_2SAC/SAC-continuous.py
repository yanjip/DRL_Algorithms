...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_width, max_action):
        ...
        self.log_std_layer = nn.Linear(hidden_width, action_dim) #输出均值 mean 和对数标准差 log_std
    ...
    def forward(self, x, deterministic=False, with_logprob=True):
        ...
        dist = Normal(mean, std)  # Generate a Gaussian distribution  计算标准差 std，并使用这两个值构建正态分布对象 dist。
        ...
        a = self.max_action * torch.tanh(a)  # 使用 torch.tanh 将输出的动作缩放到 [-max_action, max_action] 范围内。
