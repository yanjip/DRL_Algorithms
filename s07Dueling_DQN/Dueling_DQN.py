...
import numpy as np
from s07Dueling_DQN.buffer import ReplayBuffer
...
class DuelingDQN:
    ...
    # 减少探索因子epsilon的值的，它表示每次智能体与环境交互后，
    # epsilon的值会减去一个衰减率eps_dec，直到达到一个最小值eps_min
    ...
    def learn(self):
        ...
        with T.no_grad():
            ...
            # 把终止状态的Q值设为0.0，因为终止状态没有未来奖励。
            q_[terminals_tensor] = 0.0
            # q_张量在最后一个维度上的最大值和对应的索引的，它表示对于每个状态，找出最大的Q值和对应的动作，用来指导智能体的行为选择。
        ...
        self.update_network_parameters()  #然后更新target网络
